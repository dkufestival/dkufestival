...
Node.js 22.13 이상이 필요합니다. 메인 프로젝트에서는 npm으로 실행할 수 있습니다.
...
메인 프로젝트 루트에서 실행할 때는 다음 명령을 사용합니다.

```bash
npm run basketball
```

브라우저에서 `http://localhost:5175`로 접속합니다. 참가자 화면의 `게임 > 농구게임 입장` 버튼도 이 주소로 연결됩니다.
